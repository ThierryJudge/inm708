...
if __name__ == '__main__':
    ...
    parser.add_argument("--adam_lr", type=float, default=1e-1)
    ...
    figure, axes = plt.subplots(len(optims), 3)
    ...
    plt.figure()
    plt.plot(test_errors.T, label=optim_names)
    plt.legend()
    ...
    axes[0].plot(test_errors.T, label=optim_names)
    axes[0].legend()

    for i in range(len(names)):
        axes[i+1].set_title(names[i])
        axes[i+1].plot(variable_histories[i].T, label=optim_names)
