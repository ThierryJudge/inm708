...
if __name__ == '__main__':
    ...
    parser.add_argument("--fmri_seg", type=str, default='seg_cube_like.nii.gz',
                        help="Path to the segmentation file (.nii)")
    parser.add_argument("--streamlines", type=str, default='filtered_tractogram.trk',
                        help="Path to the input streamline file (.nii)")
    parser.add_argument("--dmri_path", type=str, default='Data/dmri.nii.gz', help="Path to the input dmri file (.nii)")
    parser.add_argument("--start_only", action='store_true', help='Only consider streamlines that start in regions')
    ...
    # save_tractogram(tractogram, f"new_tractogram.trk", bbox_valid_check=True)
    # exit(0)
